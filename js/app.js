...
function handleAuthState(session){
    ...
    if(session && session.user){
        ...
        document.getElementById("userGreetName").textContent =
...
function goPage(name) {
  ...
  if(name==="archive") document.getElementById("navArchive").classList.add("active");
  if(name==="gift") document.getElementById("navGift").classList.add("active");
  if(name==="reminder") document.getElementById("navReminder").classList.add("active");
  render();
}
...
function renderArchive(){
    ...
    wrap.innerHTML = sortedKeys.map(key => {
        ...
        <div class="budget-card" style="margin:0 0 12px;cursor:pointer;" onclick="toggleArchiveMonth('${key}')">
            ...
            <div style="margin-top:10px;"><b>${formatRupiah(total)}</b></div>
...
function reminderStatusOf(r){
    ...
    if(r.monthly_amount > 0){
        // Model cicilan: kalau ada total hutang dan udah abis, berarti lunas total.
        if(r.total_amount !== null && r.total_amount !== undefined && r.total_amount <= 0) return "Lunas";
    } else {
...
async function processRecurringReminders(){
    ...
    const toRoll = reminders.filter(r =>
        r.monthly_amount > 0 &&
        (r.paid_amount || 0) >= r.monthly_amount &&
        !(r.total_amount !== null && r.total_amount !== undefined && r.total_amount <= 0)
    );
    ...
    for(const r of toRoll){
        ...
        const nextDue = addOneMonth(r.due_date);

        const updates = { due_date: nextDue, paid_amount: 0 };

        if(r.total_amount !== null && r.total_amount !== undefined){
            updates.total_amount = Math.max(r.total_amount - r.monthly_amount, 0);
        }
        ...
        if(!error){
            r.due_date = nextDue;
            r.paid_amount = 0;
            if(updates.total_amount !== undefined) r.total_amount = updates.total_amount;
        }
    }
}
...
function reminderCard(r){
    ...
    const cycleTarget = isInstallment ? r.monthly_amount : r.total_amount;
    const percent = cycleTarget ? Math.min((paid / cycleTarget) * 100, 100) : 0;
    const remain = cycleTarget ? Math.max(cycleTarget - paid, 0) : 0;
    ...
    <div class="budget-card" style="${bg}">
        ...
        <div style="margin-top:4px;">
            Sisa Total Hutang <b>${formatRupiah(r.total_amount)}</b>
        </div>` : ""}
        <div style="margin-top:4px;">
            Dibayar Bulan Ini <b>${formatRupiah(paid)}</b>
        </div>
        ` : `
        <div style="margin-top:14px;">
            Total <b>${formatRupiah(r.total_amount)}</b>
        </div>
        <div style="margin-top:4px;">
        ...
        <div style="margin-top:8px;">
            Sisa ${isInstallment ? "Cicilan Bulan Ini" : ""} <b>${formatRupiah(remain)}</b>
        </div>
...
function renderHomeDashboards(){
    ...
    if(reminderBox){
        ...
        const total = dueToday.reduce((sum,r) => {
            const target = r.monthly_amount > 0 ? r.monthly_amount : (r.total_amount || 0);
            return sum + Math.max(target - (r.paid_amount||0), 0);
        }, 0);
