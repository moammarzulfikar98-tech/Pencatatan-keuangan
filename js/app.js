...
function goalCard(g){
    ...
    <div class="budget-card">
        ...
        <div class="budget-sub">
            ${g.category ? g.category + " · " : ""}Target ${g.target_date}
        </div>
...
function reminderCard(r){
    ...
    const hasTenor = r.tenor_bulan !== null && r.tenor_bulan !== undefined && r.tenor_bulan > 0;
    ...
    let cicilanKe = 0;
    if(isInstallment){
        cicilanKe = Math.floor(paid / r.monthly_amount);
        if(hasTenor) cicilanKe = Math.min(cicilanKe, r.tenor_bulan);
    }
    ...
    <div class="budget-card" style="${bg}">
        ...
        <div style="margin-top:4px;">
            Cicilan ke <b>${cicilanKe}${hasTenor ? " / " + r.tenor_bulan : ""}</b>
        </div>
        ` : ""}
...
function openReminderModal(){
    ...
    document.getElementById("reminderTenor").value = "";
...
async function saveReminder(){
    ...
    const tenorInput = document.getElementById("reminderTenor").value;
    const tenor_bulan = tenorInput ? Number(tenorInput) : null;
    ...
    const { error } = await supabaseClient
        .from("payment_reminders")
        .insert([{
            title, total_amount, paid_amount: 0, monthly_amount, tenor_bulan, due_date, category, note, is_recurring,
            user_id: currentUser.id
        }]);
